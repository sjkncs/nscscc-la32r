...
module myCPU (
    ...
    wire        id_pred_taken;       // ID阶段的BPU预测方向
    wire [31:0] id_pred_target;      // ID阶段的BPU预测目标地址
    wire        id_is_call   ;       // ID阶段是否是call指令 (RAS)
    wire        id_is_ret    ;       // ID阶段是否是ret指令 (RAS)
    // EX Stage
    ...
    wire        ex_pred_taken;       // EX阶段的BPU预测方向 (流水线回传)
    wire [31:0] ex_pred_target;      // EX阶段的BPU预测目标 (流水线回传)
    wire        ex_is_call   ;       // EX阶段是否是call (RAS push)
    wire        ex_is_ret    ;       // EX阶段是否是ret (RAS pop)
    // MEM Stage
    ...
    wire        div_suspend  ;      // 除法器暂停信号
    wire        div_done     ;      // 除法器完成信号
    wire        load_use     ;      // Load-Use冒险检测信号
    wire        pl_suspend    = ldst_suspend | div_suspend;  // 流水线暂停信号
    wire        load_use_stall = load_use & !pl_suspend;     // Load-Use暂停(仅1周期bubble)
    // 出现多周期指令(访存、乘除法)暂停取指, 多周期操作结束后继续取指
    wire        pause_ifetch  = (ldst_suspend | div_suspend | load_use_stall) & !(ldst_done | div_done);
    // 检测pause_ifetch下降沿: 覆盖所有暂停结束场景(包括unaligned load+load-use边界case)
    reg         pause_ifetch_r;
    always @(posedge cpu_clk or negedge cpu_rstn)
        if (!cpu_rstn) pause_ifetch_r <= 1'b0;
        else           pause_ifetch_r <= pause_ifetch;
    wire        resume_ifetch = ldst_done | div_done | (pause_ifetch_r & !pause_ifetch);
