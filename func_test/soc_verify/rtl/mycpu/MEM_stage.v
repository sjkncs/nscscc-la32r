...
module MEM_stage (
    ...
    wire entering_mem_ldst = !pl_suspend & ex_valid &
                             ((ex_wd_sel == `WD_RAM) | (ex_ram_we != `RAM_WE_N)) & !ldst_unalign;

    always @(posedge cpu_clk or negedge cpu_rstn) begin
        if (!cpu_rstn)
            mem_is_ld_st <= 1'b0;
        else if (!mem_is_ld_st | daccess_valid | daccess_wresp)
            mem_is_ld_st <= entering_mem_ldst;
    end
    ...
    always @(*) begin
        case (mem_wd_sel)
            ...
            `WD_PC4: mem_wd = mem_pc + 32'h4;
            `WD_EXT: mem_wd = mem_ext;
            default: mem_wd = 32'h0;
        endcase
    end
