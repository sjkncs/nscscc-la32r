...
module ID_stage (
    ...
    input  wire         if_pred_taken,      // IF阶段BPU预测是否跳转
    input  wire [31:0]  if_pred_target,     // IF阶段BPU预测目标地址
    ...
    output wire         id_pred_taken,       // ID阶段的BPU预测方向
    output wire [31:0]  id_pred_target,      // ID阶段的BPU预测目标地址
    output wire         id_is_call   ,       // BL或JIRL(rd!=0), 用于RAS push
    output wire         id_is_ret    ,       // JIRL rd=0,rj=r1, 用于RAS pop
    // Data Forward
    ...
    assign      id_is_ld_st = id_valid & ((id_wd_sel == `WD_RAM) | (id_ram_we != `RAM_WE_N));
    // RAS: call = BL or JIRL(rd!=0), ret = JIRL(rd=0, rj=r1)
    assign      id_is_call = id_is_br_jmp & id_rf_we & (id_wd_sel == `WD_PC4) & (id_wR != 5'h0);
    assign      id_is_ret  = (id_npc_op == `NPC_JIRL) & (id_inst[4:0] == 5'd0) & (id_inst[9:5] == 5'd1);
    ...
    IF_ID u_IF_ID (
        ...
        .valid_in       (if_valid      ),
        .pc_in          (if_pc         ),
        .pred_taken_in  (if_pred_taken ),
        .pred_target_in (if_pred_target),
        .pc_out         (id_pc         ),
